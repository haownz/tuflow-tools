...
class TuflowConsistencyIntegrity(QgsProcessingAlgorithm):
    ...
    DEP_MODES = ["OFF", "List this run only (-cL)", "List ALL events/scenarios (-pmL)"]
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFile(
                self.PARAM_TUFLOW_EXE,
                "Path to TUFLOW executable (e.g., TUFLOW_iSP_w64.exe)",
                behavior=QgsProcessingParameterFile.File,
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterFolderDestination(
                self.PARAM_CHECKFOLDER,
                "Check files folder (used in preview block)",
                defaultValue=d["check_folder"],
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.PARAM_INCLUDE,
                "Include list (space-separated)",
                defaultValue=d["include"],
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.PARAM_EXCLUDE,
                "Exclude list (space-separated)",
                defaultValue=d["exclude"],
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.PARAM_PREFIX,
                "Prefix for check files (optional)",
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.PARAM_SET_GIS_PROJ_CHECK,
                "Set GIS Projection Check == ON",
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.PARAM_SET_MI_SAVE_DATE,
                "Check MI Save Date (LEAVE/OFF/WARNING)",
                options=["LEAVE", "OFF", "WARNING"],
                defaultValue=d["mi_save_date"],
            )
        )

        self.addParameter(
            QgsProcessingParameterEnum(
                self.PARAM_SET_MI_SAVE_EXT,
                "Check MI Save Ext (LEAVE/OFF/WARNING)",
                options=["LEAVE", "OFF", "WARNING"],
                defaultValue=d["mi_save_ext"],
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.PARAM_PREVIEW_ONLY,
                "Preview only (do NOT edit TCF)",
        ...
        self.addParameter(
            QgsProcessingParameterFileDestination(
                self.PARAM_REPORT_FILE,
                "Optional preview report (*.txt)",
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.PARAM_RUN_TEST,
                "Run TUFLOW test mode (-t) [no TCF edits]",
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.PARAM_RUN_1D_SCAN,
                "Scan loaded 1D layers for basic integrity issues",
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.PARAM_DEP_MODE,
                "Dependencies mode",
                options=self.DEP_MODES,
                defaultValue=d["dep_mode"],
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterFileDestination(
                self.PARAM_DEP_CSV,
                "Dependencies CSV report",
                fileFilter="CSV files (*.csv);;All files (*.*)",
                defaultValue=d["dep_csv"],
                optional=True,
            )
        )
        ...
        self.addOutput(
            QgsProcessingOutputString("dependencies_summary", "Dependencies summary")
        )

    # ===== Utilities =====
    def _compose_preview_block(
        self,
        tcf_path,
        check_folder,
        mode,
    ...
    def _run_tuflow(self, exe_path, args, feedback):
        ...
        cmd = [exe_path] + args
        feedback.pushInfo(
            "Running: " + " ".join(f'"{a}"' if " " in a else a for a in cmd)
        )
        try:
    ...
    def _run_tuflow_test(self, exe_path, tcf_path, feedback):
        code, out, err = self._run_tuflow(
            exe_path, ["-t", "-b", "-nc", tcf_path], feedback
        )
        if code is None:
            return None
        ...
        with open(os.path.join(log_dir, f"{base}_test_stderr.txt"), "wb") as f:
            f.write(err or b"")
        feedback.pushInfo(
            f"TUFLOW test completed (exit code={code}). Logs saved to: {log_dir}"
        )
        return code
    ...
    def _audit_dependencies(self, exe_path, tcf_path, dep_mode, csv_path, feedback):
        ...
        if dep_mode == 1:  # -cL (this run only)
            code, out, err = self._run_tuflow(
                exe_path, ["-cL", "-b", "-nc", tcf_path], feedback
            )
            if code is None:
            ...
            code, out, err = self._run_tuflow(
                exe_path, ["-pmL", "-b", "-nc", tcf_path], feedback
            )
            if code is None:
                return "Dependencies: TUFLOW not executed."
        ...
        for p in files:
            ...
            rows.append(
                [p_norm, "OK" if exists else "MISSING", size, mtime, ftype, gtype, crs]
            )
        ...
        total = len(
            [
                r
                for r in rows
                if not r[0].lower().endswith(".dbf")
                and not r[0].lower().endswith(".shx")
            ]
        )
        summary = (
        ...
        for lyr in layers:
            ...
            for f in lyr.getFeatures():
                ...
                if geom is None or geom.isEmpty():
                    ...
                    feat.setAttributes(
                        [lyr.name(), "EMPTY_GEOMETRY", str(f.id()), None]
                    )
                ...
                if id_field:
                    ...
                    if val in seen:
                        ...
                        feat.setFields(fields)
                        feat.setAttributes(
                            [lyr.name(), "DUPLICATE_ID", str(f.id()), str(val)]
                        )
                        try:
                            pt = (
                                geom.centroid().asPoint()
                                if geom and not geom.isEmpty()
                                else lyr.extent().center()
                            )
                        except Exception:
                            pt = lyr.extent().center()
                        feat.setGeometry(
                            QgsGeometry.fromWkt(f"POINT({pt.x()} {pt.y()})")
                        )
                        sink.addFeature(feat)
                    else:
                        seen.add(val)
    ...
    def processAlgorithm(
        ...
        check_folder = self.parameterAsString(
            parameters, self.PARAM_CHECKFOLDER, context
        )
        ...
        set_proj_check = self.parameterAsBoolean(
            parameters, self.PARAM_SET_GIS_PROJ_CHECK, context
        )
        mi_save_date = self.parameterAsEnum(
            parameters, self.PARAM_SET_MI_SAVE_DATE, context
        )
        mi_save_ext = self.parameterAsEnum(
            parameters, self.PARAM_SET_MI_SAVE_EXT, context
        )
        preview_only = self.parameterAsBoolean(
            parameters, self.PARAM_PREVIEW_ONLY, context
        )
        report_path = self.parameterAsFileOutput(
            parameters, self.PARAM_REPORT_FILE, context
        )
        ...
        save_defaults = self.parameterAsBoolean(
            parameters, self.PARAM_SAVE_DEFAULTS, context
        )
        ...
        if report_path:
            try:
                ...
                with open(report_path, "w", encoding="utf-8") as rf:
                    ...
                    rf.write(
                        f"GIS Projection Check: {'ON' if set_proj_check else 'LEAVE'}\n"
                    )
                    rf.write(
                        f"Check MI Save Date: {['LEAVE', 'OFF', 'WARNING'][mi_save_date]}\n"
                    )
                    rf.write(
                        f"Check MI Save Ext: {['LEAVE', 'OFF', 'WARNING'][mi_save_ext]}\n"
                    )
            except Exception as e:
        ...
        dep_summary = ""
        if dep_mode != 0:
            dep_summary = self._audit_dependencies(
                exe_path, tcf_path, dep_mode, dep_csv, feedback
            )
