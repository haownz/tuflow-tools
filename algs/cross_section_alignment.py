...
class CrossSectionAlignmentDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.cs_arrow_rubber_band = QgsRubberBand(
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["Raster Layer", "Elevation (m)", "Style", "Sample"]
        )
    ...
    def update_chainage(self):
        ...
        if not layer_valid:
            crs = self.crs.authid()
            self.chainage_layer = QgsVectorLayer(
                f"LineString?crs={crs}", "Chainage Lines", "memory"
            )
        ...
        for d in dists:
            ...
            if geom:
                ...
                if sample_layer and sample_layer.isValid():
                    ...
                    if pt_geom:
                        pt = pt_geom.asPoint()
                        val, ok = self._sample_raster(
                            sample_layer, QgsPointXY(pt.x(), pt.y())
                        )
    ...
    def _apply_chainage_labeling(self, use_blue_right: bool):
        ...
        p1.setProperty(
            QgsPalLayerSettings.LabelRotation, QgsProperty.fromExpression(rotation_expr)
        )
    ...
    def on_move_up(self):
        row = self.table.currentRow()
        if row > 0:
            self.raster_layers[row], self.raster_layers[row - 1] = (
                self.raster_layers[row - 1],
                self.raster_layers[row],
            )
            self.refresh_table()
    ...
    def refresh_table(self):
        ...
        for i, item_data in enumerate(self.raster_layers):
            ...
            combo.currentTextChanged.connect(
                lambda text, row=i: self.on_style_changed(row, text)
            )
    ...
    def on_sample_checked(self, item):
        ...
        for r in range(self.table.rowCount()):
            ...
            if cell:
                new_state = (
                    Qt.Checked
                    if r == checked_row and item.checkState() == Qt.Checked
                    else Qt.Unchecked
                )
    ...
    def refresh_long_section_data(self):
        ...
        self.long_section_data = {"dists": dists, "values": results}
    ...
    def refresh_plots(self):
        ...
        self.marker_line = self.ax_long.axvline(
            x=0, color="r", linestyle="--", alpha=0.8, zorder=10
        )
    ...
    def refresh_cross_section_plot(self):
        ...
        width = self.spin_width.value()
        cs_geom = self.get_cross_section_geom(
            self.current_cursor_dist, width, self.reverse_offset
        )
                ...
                if layer.id() in results:
                    ...
                    if self.show_water_levels and style not in ("EGL", "PGL"):
                        ...
                        if not np.isnan(z_val):
                            wls_to_draw.append(
                                {"val": z_val, "color": c, "name": layer.name()}
                            )
    ...
    def _draw_water_levels(self, ax, wls_list):
        ...
        for item in wls_list:
            ...
            w_text = max(len(line) for line in lines) * 6 + 10
            ...
            for r in radii:
                for theta in angles:
                    ...
                    if not any(
                        ...
                        for (px1, px2, py1, py2) in placed_labels
                    ):
        ...
        try:
            with PdfPages(filepath) as pdf:
                ...
                progress = QProgressDialog(
                    "Generating PDF...", "Cancel", 0, len(dists) + 2, self
                )
                ...
                align_layer = QgsVectorLayer(
                    f"LineString?crs={crs_auth}", "Alignment", "memory"
                )
                af = QgsFeature()
                ...
                if self.long_section_data:
                    ...
                    colors = plt.cm.tab10(
                        np.linspace(0, 1, max(1, len(self.raster_layers)))
                    )
                ...
                for i, d in enumerate(dists):
                    ...
                    if cs_geom:
                        layers = [x["layer"] for x in self.raster_layers]
                        s_dists, results = self.sample_line(
                            cs_geom, layers, num_points=100
                        )
                        ...
                        for j, item_data in enumerate(self.raster_layers):
                            ...
                            if layer.id() in results:
                                ...
                                if self.show_water_levels and style not in (
                                    "EGL",
                                    "PGL",
                                ):
                                    idx = np.abs(np.array(offsets)).argmin()
                        ...
                        ax.set_title(
                            f"Cross Section - Chainage: {d:.1f} m", fontsize=10
                        )
                        ax.set_xlabel("Offset (m)", fontsize=8)
                        ax.set_ylabel("Elevation (m)", fontsize=8)
                        ax.grid(True, alpha=0.3)
                        ax.axvline(
                            0, color="r", linestyle="--", alpha=0.5, linewidth=0.8
                        )
    ...
    def save_settings(self):
        ...
        s.setValue(
            "tuflow_tools/cross_section/chainage_on", self.btn_chainage.isChecked()
        )

        if self.alignment_geom:
            s.setValue(
                "tuflow_tools/cross_section/alignment_wkt", self.alignment_geom.asWkt()
            )
        ...
        for item in self.raster_layers:
            try:
                r_list.append(
                    f"{item['layer'].id()}|{item['style']}|{item['layer'].source()}"
                )
        ...
        if self.chainage_layer:
            try:
                s.setValue(
                    "tuflow_tools/cross_section/chainage_layer_id",
                    self.chainage_layer.id(),
                )
            except RuntimeError:
                pass
    ...
    def load_settings(self):
        ...
        try:
            self.spin_width.setValue(
                float(s.value("tuflow_tools/cross_section/width", 100.0))
            )
            self.spin_interval.setValue(
                float(s.value("tuflow_tools/cross_section/interval", 50.0))
            )
            self.btn_reverse.setChecked(
                s.value("tuflow_tools/cross_section/reverse", False, type=bool)
            )
            self.btn_wl.setChecked(
                s.value("tuflow_tools/cross_section/water_levels", False, type=bool)
            )
            self.btn_chainage.setChecked(
                s.value("tuflow_tools/cross_section/chainage_on", False, type=bool)
            )
        except Exception:
            pass
        ...
        if c_id:
            layer = QgsProject.instance().mapLayer(c_id)
            if layer and isinstance(layer, QgsVectorLayer):
                self.chainage_layer = layer
        ...
        for entry in r_list:
            ...
            if layer and isinstance(layer, QgsRasterLayer) and layer.isValid():
                self.raster_layers.append({"layer": layer, "style": style})
        self.refresh_table()
