...
def _write_debug_log(content):
    ...
    try:
        debug_dir = os.path.join(
            os.path.expanduser("~"), "Documents", "tuflow_tools_debug"
        )
...
class TCFSelectionPage(QWizardPage):
    ...
    def browse_folder(self):
        d = QFileDialog.getExistingDirectory(
            self, "Select TCF Folder", self.run_path or ""
        )
        if d:
            self.run_path = d
            self.refresh_file_list()
...
class ScenarioSelectionPage(QWizardPage):
    ...
    def init_ui(self):
        ...
        self.scenarios_table.horizontalHeader().setSectionResizeMode(
    ...
    def initializePage(self):
        ...
        try:
            ...
            all_scenarios, all_events = self._extract_scenarios_and_events(
                selected_tcfs, wizard
            )
    ...
    def _extract_scenarios_and_events(self, selected_tcfs, wizard):
        ...
        for tcf in selected_tcfs:
            ...
            try:
                ...
                if DEBUG_MODE:
                    _write_debug_log(
                        f"--- Tidied TCF: {os.path.basename(tcf)} ---\n"
                        + "\n".join(cleaned_lines)
                    )

                variables = self._extract_variables_from_tcf(cleaned_lines)
                cleaned_lines_with_vars = [
                    self._substitute_variables(line, variables)
                ...
                for line in cleaned_lines_with_vars:
                    if "event file" in line.lower() and "==" in line:
                        ep = os.path.join(
                            tcf_dir, line.split("==")[1].strip().strip("\"'")
                        )
                        if os.path.exists(ep):
                            try:
                                with open(
                                    ep, "r", encoding="utf-8", errors="ignore"
                                ) as ef:
                                    ef_lines = self._clean_tcf_content(ef.read())
                                    if DEBUG_MODE:
                                        _write_debug_log(
                                            f"--- Tidied Event File: {os.path.basename(ep)} ---\n"
                                            + "\n".join(ef_lines)
                                        )
                                    for el in ef_lines:
                                        if (
                                            el.lower().startswith("define event")
                                            and "==" in el
                                        ):
    ...
    def extract_heuristic(self, filename, event_list):
        ...
        for ev in sorted_ev:
            ...
            if re.search(pattern, working_string, re.IGNORECASE):
                ...
                working_string = re.sub(
                    pattern, "|", working_string, count=1, flags=re.IGNORECASE
                )
        ...
        for part in parts:
            if not part:
                continue
            if part.lower() in ignored_keywords:
                continue
            if any(part.lower() == ev.lower() for ev in event_list):
                continue
            scenarios.add(part)
        ...
        for line in cleaned_lines:
            if "set variable" in line.lower() and "==" in line:
                ...
                if len(parts) == 2:
                    ...
                    var_name = re.sub(
                        r"(?i)^\s*set\s+variable\s+", "", parts[0]
                    ).strip()
                            ...
                            if os.path.exists(head):
                                try:
                                    ...
                                    subdirs = [
                                        ...
                                        if os.path.isdir(os.path.join(head, d))
                                    ]
                                    for sd in subdirs:
        ...
        for tcf in wizard.tcf_page.get_selected_files():
            ...
            for fmt in grid_formats:
                for line in content:
                    line = line.split("!")[0].strip()
                    if (
                        fmt.lower() + " map output data types" in line.lower()
                    ) and "==" in line:
...
class PreviewPage(QWizardPage):
    ...
    def init_ui(self):
        ...
        self.empty_warning_label = QLabel(
            "⚠️ Warning: No matching grid files were found! The simulation may not have results yet, or the model path in the TCF might be incorrect."
        )
        self.empty_warning_label.setStyleSheet(
            "color: #d9534f; font-weight: bold; padding: 5px;"
        )
    ...
    def initializePage(self):
        ...
        try:
            ...
            selected_datatypes = [
                (
                    wizard.output_page.output_table.item(r, 1).text(),
                    wizard.output_page.output_table.item(r, 2).text(),
                )
            ...
            if wizard.grids_folder_cache:
                ...
                tcf_structures = []
                for tcf in wizard.tcf_page.get_selected_files():
                    st = wizard.scenario_page.extract_tcf_structure(
                        os.path.basename(tcf)
                    )
                    if st:
                        tcf_structures.append(st)

                all_tifs = _find_tif_files(wizard.grids_folder_cache)
                debug_lines = []
                if DEBUG_MODE:
                    debug_lines.append(
                        f"=== TUFLOW Tools Debug: Grid Files ===\nFolders: {wizard.grids_folder_cache}\nTotal TIFs: {len(all_tifs)}"
                    )

                for fpath in all_tifs:
                    is_match = self.matches(
                    ...
                    if DEBUG_MODE:
                        debug_lines.append(
                            f"{'[MATCH]' if is_match else '[SKIP ]'} {os.path.basename(fpath)}"
                        )
...
class LoadGridOutputAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self):
        return "load_grid_output"

    def displayName(self):
        return "1 - Load Grid Output"
    ...
    def createInstance(self):
        return LoadGridOutputAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if wizard.exec_() == QWizard.Accepted:
            ...
            loaded_names = []
            sorted_files = sorted(
                wizard.preview_page.grid_files_to_load,
            ...
            for idx, fpath in enumerate(reversed(sorted_files)):
                ...
                try:
                    lyr = QgsRasterLayer(
                        fpath, os.path.splitext(os.path.basename(fpath))[0]
                    )
                    if lyr.isValid():
                        ...
                        feedback.reportError(f"Failed to load: {fpath}")
                except Exception as e:
                    feedback.reportError(f"Error: {e}")

            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_scenario_layers", json.dumps(loaded_names)
            )
            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_raster_files",
                json.dumps(wizard.preview_page.grid_files_to_load),
            )
            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_load_time", QDateTime.currentDateTime().toString()
            )
