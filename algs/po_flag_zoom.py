...
class POFlagZoomAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return (
            "Sets OV/ZO integer flags using PO_Line_OV.csv (row 1) and PO_Line_ZO.csv "
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterString(self.P_ID, "ID field", defaultValue="ID")
        )
        self.addParameter(
            QgsProcessingParameterFile(
                self.P_RESULTS,
                "Results folder (where PO-Line_OV/ZO.csv live)",
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if v is None:
            ...
            if not isinstance(r, QgsRasterLayer):
                raise QgsProcessingException(
                    "Raster is required when no PO line vector is provided."
                )
        ...
        if id_field not in v.fields().names():
            raise QgsProcessingException(
                "Field '{}' not found in layer '{}'.".format(id_field, v.name())
            )
        ...
        if not ov_path and not zo_path:
            raise QgsProcessingException(
                "Could not find PO_Line_OV.csv / PO_Line_ZO.csv."
            )
        if ov_path and not ov_path.exists():
            feedback.reportError("OV CSV not found: {}".format(ov_path), fatal=False)
        if zo_path and not zo_path.exists():
            feedback.reportError("ZO CSV not found: {}".format(zo_path), fatal=False)

        ov_ids = read_row_from_csv(ov_path, 0) if ov_path and ov_path.exists() else []
        zo_ids = (
            read_row_from_csv(zo_path, zo_row - 1)
            if zo_path and zo_path.exists()
            else []
        )
        ...
        feedback.pushInfo(
            "Updated OV/ZO flags on {} features (ZO row={}).".format(updated, zo_row)
        )
        return {}

    def createInstance(self):
        return POFlagZoomAlgorithm()
