...
def find_corresponding_rasters(grid_layer_name, raster_dir, base_name, grid_type):
    ...
    """
    raster_map = {"Depth": None, "Level": None, "Velocity": None}
    ...
    for letter, key in [("d", "Depth"), ("h", "Level"), ("v", "Velocity")]:
        ...
        search_name = re.sub(
            rf"_{grid_type}_", f"_{letter}_", grid_layer_name, flags=re.IGNORECASE
        )
        search_name = re.sub(
            rf"_{grid_type}\.", f"_{letter}.", search_name, flags=re.IGNORECASE
        )
...
def sample_rasters_at_points(
    ...
    try:
        ...
        output_layer = QgsVectorLayer(
            uri, f"{input_points_layer.name()}_sampled", "memory"
        )
            ...
            if path and os.path.exists(path):
                ...
                try:
                    ...
                    if raster.isValid():
                        ...
                        feedback.pushInfo(
                            f"    ⚠ Warning: Invalid raster {key}: {os.path.basename(path)}"
                        )
                except Exception as e:
                    if feedback:
                        feedback.pushInfo(
                            f"    ⚠ Warning: Could not load {key} raster: {e}"
                        )

        # Sample at each input point with batch processing
        terrain_providers = [
            layer.dataProvider() for layer in terrain_layers if layer.isValid()
        ]
        batch_size = 500
        batch_features = []
        point_id = 1
        ...
        if feedback:
            feedback.reportError(
                f"    ✗ Exception in sample_rasters_at_points: {str(e)}"
            )
        return None
    ...
    try:
        ...
        if os.path.exists(output_path):
            if overwrite_mode == "skip":
                if feedback:
                    feedback.pushInfo(
                        f"  ⊘ Skipping (file exists): {os.path.basename(output_path)}"
                    )
                return True, None, True
            elif overwrite_mode == "overwrite":
                if feedback:
                    feedback.pushInfo("  ↻ Overwriting existing file...")
...
class FileOverwriteDialog(QDialog):
    ...
    def init_ui(self, existing_files):
        layout = QVBoxLayout()
        layout.addWidget(
            QLabel(f"Found {len(existing_files)} existing output file(s):")
        )
...
class LoadSamplePointsInputDialog(QDialog):
    ...
    def init_ui(self):
        ...
        layout.addWidget(
            QLabel(
                "Step 2: Select DEM/Terrain raster layers (Drag to reorder, Last wins!)"
            )
        )
    ...
    def detect_and_populate_grid_layers(self):
        ...
        for layer in project.mapLayers().values():
            if isinstance(layer, QgsRasterLayer):
                ...
                if re.search(r"_[dhv]_", layer.name(), re.IGNORECASE) or re.search(
                    r"_[dhv]\.", layer.name(), re.IGNORECASE
                ):
...
class LoadSamplePointsAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for grid_layer_name, grid_layer in grid_layers:
            try:
                base_name, grid_type = extract_scenario_base_from_grid_layer(
                    grid_layer_name
                )
        ...
        feedback.pushInfo("\n" + "=" * 70)
        feedback.pushInfo(
            "Step 3: Processing grid layers and generating sample point files"
        )
        feedback.pushInfo("=" * 70)
        ...
        for idx, (grid_layer_name, grid_layer) in enumerate(grid_layers):
            ...
            try:
                ...
                if base_name in processed_bases:
                    feedback.pushInfo(
                        f"  → Skipping: Scenario '{base_name}' already processed in this batch"
                    )
                    continue
                ...
                feedback.pushInfo(
                    f"  Grid type: {grid_type} ({type_names.get(grid_type, 'Unknown')})"
                )

                # Get raster directory
                raster_uri = grid_layer.dataProvider().dataSourceUri()
                if not raster_uri:
                    feedback.pushInfo("  ⚠ Could not determine raster directory")
                    continue
                raster_path = os.path.normpath(raster_uri)
                raster_dir = os.path.dirname(raster_path)
                ...
                feedback.pushInfo(
                    f"  → Searching for d/h/v rasters in: {os.path.basename(raster_dir)}"
                )
                for key, path in raster_map.items():
                    if path:
                        ...
                        feedback.pushInfo(
                            f"    ⚠ {key}: NOT FOUND (field will be empty)"
                        )
                ...
                if not sample_layer:
                    feedback.reportError("    ✗ Failed to sample raster values")
                    continue
                ...
                feedback.reportError(
                    f"  ✗ Error processing [{current}/{total}]: {str(e)}"
                )
                gc.collect()
        ...
        if newly_generated_files:
            feedback.pushInfo(
                f"  ({processed_count} newly generated, {len(all_files_to_load) - processed_count} existing)"
            )
        ...
        sorted_files = sorted(
            all_files_to_load, key=lambda x: os.path.basename(x).lower(), reverse=True
        )
                ...
                if os.path.exists(plot_p_path):
                    ...
                    if lyr and lyr.isValid():
                        ...
                        try:
                            ...
                            feedback.pushInfo("  ✓ Loaded with style")
                        except Exception as style_error:
                            feedback.pushInfo(
                                f"  ✓ Loaded (style skipped: {str(style_error)})"
                            )
        ...
        try:
            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_sample_layers", json.dumps(loaded_layers)
            )
            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_sample_files", json.dumps(all_files_to_load)
            )
            QgsExpressionContextUtils.setGlobalVariable(
                "tuflow_latest_sample_time", QDateTime.currentDateTime().toString()
            )
        except Exception:
            pass
