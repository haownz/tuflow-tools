...
class LandCoverAddFieldsAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if (gen_2d_mat or gen_2d_soil) and model_path:
            ...
            if gen_2d_mat:
                out_mat_path = self.parameterAsFileOutput(
                    parameters, self.OUT_MAT, context
                )
            if gen_2d_soil:
                out_soil_path = self.parameterAsFileOutput(
                    parameters, self.OUT_SOIL, context
                )
        ...
        if class_id_idx == -1:
            raise QgsProcessingException(
                self.tr("Input layer does not contain a 'ClassID' field.")
            )
