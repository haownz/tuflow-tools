...
def _parse_time_token(token, unit_hint=None):
    ...
    if m:
        ...
        mult = (
            ...
            else (60.0 if unit.startswith("min") else 1.0)
        )
        return val * mult
    return None
...
def compute_volume_map_for_csv(path, skip_cols):
    ...
    if col_oriented:
        ...
        for j in range(skip_cols, len(header)):
            ...
            for k in station_variants(clean):
                vol_map[k] = vol
...
def update_qp_for_layer(po_layer, rel_dir="../csv", skip_cols=2, feedback=None):
    if not po_layer or not po_layer.isValid():
        return None, 0, 0, []
    ...
    vol_maps = {
        tag: compute_volume_map_for_csv(path, skip_cols)[0]
        for tag, path in csvs.items()
    }

    for f_name in ["QP", "QV"]:
        if mem_layer.fields().indexOf(f_name) == -1:
            mem_layer.dataProvider().addAttributes(
                [QgsField(f_name, QVariant.Double, "Double", 15, 5)]
            )
    ...
    for feat in mem_layer.getFeatures():
        ...
        for tag in ("1d", "2d"):
            for v in qp_variants:
                ...
                if val is not None:
                    entry[qp_idx] = round(float(val), 5)
                    u_qp += 1
                    break
            if qp_idx in entry:
                break
        ...
        for tag in ("1d", "2d"):
            for v in qv_variants:
                ...
                if val is not None:
                    entry[qv_idx] = round(float(val), 1)
                    u_qv += 1
                    break
            if qv_idx in entry:
                break
...
def _guess_suffix(path):
    ...
    for t in ["d", "h", "v", "q", "dt"]:
        if f"_{t}_" in fname:
            return f"_{t}_*.tif"
        if fname.lower().endswith(f"_{t}.tif"):
            return f"_{t}.tif"
    return "_d_*.tif"
...
class FileOverwriteDialog(QDialog):
    def __init__(self, existing_files, parent=None):
        ...
        for t, c in [
            ("Cancel", self.reject),
            ("Skip All", self.accept),
            ("Overwrite All", self.accept),
        ]:
            btn = QPushButton(t)
            ...
            btn.clicked.connect(
                lambda checked, arg=t.split()[0].lower(): setattr(
                    self, "user_choice", arg
                )
            )
...
class PreviewDialog(QDialog):
    def __init__(self, latest_files, parent=None):
        ...
        sel_lyt.addWidget(btn_all)
        sel_lyt.addWidget(btn_none)
        sel_lyt.addStretch()
...
class LoadPOLinesAlgorithm(QgsProcessingAlgorithm):
    def createInstance(self):
        return LoadPOLinesAlgorithm()

    def name(self):
        return "load_po_lines"

    def displayName(self):
        return "2 - Load PO Lines"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        latest_files_str = QgsExpressionContextUtils.globalScope().variable(
        ...
        latest_files = [
            f for f in latest_files if os.path.normpath(f).lower() in loaded_paths
        ]

        # 1b. Fallback: If no history files are active, try the current selected raster layer
        if not latest_files:
            feedback.pushInfo(
                "No active TUFLOW history files found. Checking selected layout layer..."
            )
        ...
        if not latest_files:
            feedback.reportError(
                "None of the previously loaded grid files are currently active in the Layers panel, "
        ...
        for idx, rf in enumerate(latest_files):
            ...
            if os.path.exists(qp_p) and mode == "skip":
                feedback.pushInfo("  ⊘ Skipping existing file")
            ...
            try:
                ...
                if upd_lyr:
                    if save_layer_to_shapefile(upd_lyr, qp_p, feedback)[0]:
                        ...
                        feedback.reportError("  ✗ Failed to save QP file")
                else:
                    feedback.reportError("  ✗ Failed to process layer")
            except Exception as e:
        ...
        sorted_qp_files = sorted(
            loaded_results, key=lambda x: os.path.basename(x).lower()
        )
        for qp_path in reversed(sorted_qp_files):
            ...
            if lyr and lyr.isValid():
                ...
                try:
                    ...
                    feedback.pushInfo(
                        f"Style skipped for {lyr.name()}: {str(style_error)}"
                    )
