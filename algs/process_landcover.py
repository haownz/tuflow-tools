...
class ProcessLandcoverAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("Combined Land Cover Layer")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if lc_layer is None:
            raise QgsProcessingException(
                self.invalidSourceError(parameters, self.PARAM_LC_LAYER)
            )

        imp_layer = self.parameterAsSource(parameters, self.PARAM_IMP_LAYER, context)
        if imp_layer is None:
            raise QgsProcessingException(
                self.invalidSourceError(parameters, self.PARAM_IMP_LAYER)
            )
        ...
        res_fix_lc = processing.run(
            "native:fixgeometries",
            {"INPUT": lc_param, "OUTPUT": "TEMPORARY_OUTPUT"},
            context=context,
            feedback=feedback,
            is_child_algorithm=True,
        )
        res_fix_imp = processing.run(
            "native:fixgeometries",
            {"INPUT": imp_param, "OUTPUT": "TEMPORARY_OUTPUT"},
            context=context,
            feedback=feedback,
            is_child_algorithm=True,
        )
        ...
        res_sub_lc = processing.run(
            "native:subdivide",
            {"INPUT": fixed_lc, "MAX_NODES": 256, "OUTPUT": "TEMPORARY_OUTPUT"},
            context=context,
            feedback=feedback,
            is_child_algorithm=True,
        )
        ...
        res_sub_imp = processing.run(
            "native:subdivide",
            {"INPUT": fixed_imp, "MAX_NODES": 256, "OUTPUT": "TEMPORARY_OUTPUT"},
            context=context,
            feedback=feedback,
            is_child_algorithm=True,
        )
