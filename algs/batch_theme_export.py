...
class BatchThemeExportDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        main_layout.addWidget(
            QLabel("2. Select Map Themes to Export (Drag to reorder):")
        )
    ...
    def _save_settings(self):
        settings = QSettings()
        settings.setValue(
            "tuflow_tools/batch_export/layout", self.combo_layouts.currentText()
        )
        settings.setValue("tuflow_tools/batch_export/dpi", self.spin_dpi.value())
        settings.setValue(
            "tuflow_tools/batch_export/output_path", self.txt_filepath.text()
        )
        settings.setValue(
            "tuflow_tools/batch_export/auto_open", self.chk_auto_open.isChecked()
        )
    ...
    def _restore_settings(self):
        ...
        saved_auto_open = settings.value(
            "tuflow_tools/batch_export/auto_open", True, type=bool
        )
    ...
    def _run_export(self):
        ...
        if not output_path:
            QMessageBox.warning(
                self, "Error", "Please select or enter an output file path."
            )
            return
        ...
        if not theme_map_items:
            QMessageBox.warning(
                self,
                "Error",
                "The selected layout does not have any Map Item set to 'Follow map theme'.",
            )
            return

        # Backup original theme to restore later
        original_themes = {
            map_item.uuid(): map_item.followVisibilityPresetName()
        ...
        for index, theme in enumerate(selected_themes):
            ...
            if result == QgsLayoutExporter.Success:
                ...
                QMessageBox.warning(
                    self, "Export Error", f"Failed to export theme: {theme}"
                )
        ...
        try:
            ...
            if self.chk_auto_open.isChecked():
                ...
                QMessageBox.information(
                    self,
                    "Success",
                    "Layouts successfully exported and combined into one PDF.",
                )
            ...
            for theme, pdf_path in temp_pdfs:
                safe_theme_name = "".join(
                    [c for c in theme if c.isalpha() or c.isdigit() or c == " "]
                ).rstrip()
...
class BatchThemeExportAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            "Export a layout to a combined PDF looping with a series of selected map themes."
        )
