...
class PluginSettings:
        ...
        try:
            ...
            for i, mapping in enumerate(mappings):
                if (
                    ...
                    and len(mapping) == 3
                    and all(isinstance(item, str) for item in mapping)
                ):
    ...
    def set_path_mappings(mappings: List[Tuple[str, str]]) -> None:
        ...
        for i, mapping in enumerate(mappings):
            if not (isinstance(mapping, (list, tuple)) and len(mapping) == 2):
                raise ValueError(
                    f"Mapping at index {i} must be a tuple/list of 2 strings"
                )
            if not all(isinstance(item, str) for item in mapping):
                raise ValueError(f"Mapping at index {i} must contain only strings")
    ...
    @staticmethod
    def set_auto_apply_style(enabled: bool) -> None:
        """Set the auto apply style setting."""
        PluginSettings._set_setting("auto_apply_style", enabled)
